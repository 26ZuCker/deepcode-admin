<!-- 后期尝试是否能改为函数式组件 -->
<template>
<v-speed-dial v-if="$route.name === 'schedule' || $route.name === 'project'" :left="posLeft" :right="posRight" v-model="isFabActive" fixed bottom direction="top" open-on-hover transition="slide-y-reverse-transition">
...
<v-speed-dial v-else-if="showFloatBtn" bottom right open-on-hover fixed @click="$vuetify.goTo(target, options)">
  <template v-slot:activator>
